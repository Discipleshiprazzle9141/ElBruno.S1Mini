...
One further caveat that is easy to get wrong. That figure was measured on Superwhisper's official [GGUF Q4_K_M build](https://huggingface.co/superwhisper/s1-mini-GGUF) — their model card calls it "the build the published accuracy was measured on." This library runs a different quantization, the INT4 ONNX conversion, and its accuracy has not been separately measured. So 94.8% is a real number about the reference build, not a verified property of the weights this package downloads. Quoting it as the latter would be overclaiming.

If you are not on .NET, those official GGUF builds are the better starting point — they work with llama.cpp, Ollama, and LM Studio directly. The ONNX conversion exists specifically because ONNX Runtime GenAI is what .NET can consume.
...
- Official GGUF builds (llama.cpp / Ollama / LM Studio): https://huggingface.co/superwhisper/s1-mini-GGUF
