...
.DESCRIPTION
    ...
    1. src/ElBruno.S1Mini/ElBruno.S1Mini.csproj <Version> equals -Version.
    2. README.md's "## What's New" section has exactly 5 bullet entries and the
       first one mentions "v<Version>".
    3. (Optional) if -PackageDirectory is supplied and exists, every packed
...
.PARAMETER Version
    The expected release version, e.g. "0.2.0".
...
Write-Host "Checking $csprojPath..." -ForegroundColor Cyan
...
if (-not $csprojMatch.Success) {
    $failures.Add("$csprojPath does not contain a <Version> element.")
} elseif ($csprojMatch.Groups[1].Value -ne $Version) {
    $failures.Add("$csprojPath <Version> is '$($csprojMatch.Groups[1].Value)', expected '$Version'.")
} else {
...
# --- 3. Optional packed assembly-version check --------------------------------
...
Write-Host "✅ Release version $Version is consistent across the csproj and README.md." -ForegroundColor Green
